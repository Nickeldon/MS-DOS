...
public class MSDOS {
    public static void main(String[] args) throws InterruptedException, NoSuchFieldException, IllegalAccessException, IOException{
        ...
        boolean space = false;
        int shutdown = 0;
        String passwd = "root", passwd1 = "root";
        Scanner c = new Scanner(in);
        Random r = new Random();
        long maxMemory = Runtime.getRuntime().maxMemory();
        PrintWriter ecr = new PrintWriter(new FileOutputStream("Passwdext.txt", true));
        File file = new File("Passwdext.txt");
        BufferedReader lec = new BufferedReader(new FileReader("Passwdext.txt"));
        BufferedReader pass = new BufferedReader(new FileReader("BootChoice.txt"));
        String Line = lec.readLine();
        passwd = Line;
        lec.close();
        String verify = pass.readLine();
        pass.close();
        String truechoice = "True", falsechoice = "False";
            if(verify.equals("True")){
                truechoice.equals("**True**");
            } 
            else
            falsechoice.equals("**False**");
        ...
        do {
            ...
            switch (command) {
                case "sudo" ->{
                    if(echo[1].equals("root")){
                        if(echo[2].equals("set")){
                            if(echo[3].equals("user")){
                                if(echo[4].equals("data")){
                                    out.println("-->SETTINGS----------------------------------------------------------------------------------");
                                    out.println("BOOT BEHAVIOUR - 1");
                                    String settings = c.nextLine();
                                    if(settings.equals("1")){
                                        out.println("--------------------------------------------------------------------------------------------");
                                        out.println("|");out.println("|");out.println("|");out.println("|"); out.print("     Password at boot?: " + truechoice + " / " + falsechoice); out.println("|");out.println("|");out.println("|");out.println("|");out.println("|");
                                        out.println("--------------------------------------------------------------------------------------------");
                                        String passwdchoice = c.nextLine();
                                        PrintWriter passt = new PrintWriter(new FileOutputStream("BootChoice.txt", false));
                                        if(passwdchoice.equals("True")){
                                        passt.print("True");
                                        }
                                        else
                                        passt.print("False");
                        ...
                        if(echo[2].equals("passwd")){
                            if(echo[3].equals("reset")){                                
                                Thread.sleep(1000);
                                out.print("\033[H\033[2J");
                                out.println();
                                out.print("Password: ");
                                ...
                                passwd1 = c.nextLine();
                                Thread.sleep(2000);                                
                                }while(!passwd1.equals(passwd));
                                out.println();
                                out.print("New Password: ");
                                passwd = c.next();
                                passwd1 = passwd;
                                PrintWriter ecr2 = new PrintWriter(new FileOutputStream("Passwdext.txt", false));
                                ecr2.print(passwd);
                                ecr2.close();
                                ecr.close();
                            }
    ...
    out.println(loopfinal);}}
                ...
                case "chkdsk" -> {
                    ...
                    Thread.sleep(2000);
                    String filesyst = "";
                    if(inputline.equals(extended))
                    filesyst = Eformat;
                    if(inputline.equals(logical))
                    filesyst = Dformat;
                    if(inputline.equals(primary))
                    filesyst = Cformat;
                    out.println("The disk file system type is " + filesyst + ".");
                    Thread.sleep(100);
                    out.println("The volume name is MSDOS.");
                Thread.sleep(1000);
                out.println("Execution of CHKDSK in read only mode begins");
                    out.println("Step 1: Verifying the system base file structure");
                    String f = "";
        timestop = 50;
        int s = 0;
        int min = 0;
        int h = 0;
        boolean turns = false;
        out.println("Estimated remaining time: ");
        out.print(" ");
...
String prog = "";
    ...
    out.print("\033[H\033[2J");
                    ...
                    Thread.sleep(1000);
                    out.println("Files verifying completed");
                }
